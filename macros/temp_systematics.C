...
void temp_systematics () {
  ...
  RooUnfoldResponse *rHC50 = (RooUnfoldResponse*) fres->Get("m_pt_res_HC50"); //!
  RooUnfoldResponse *rDS = (RooUnfoldResponse*) fres->Get("m_pt_res_DS"); //!
  RooUnfoldResponse *rGS = (RooUnfoldResponse*) fres->Get("m_pt_res_GS"); //!
  RooUnfoldResponse *rMS = (RooUnfoldResponse*) fres->Get("m_pt_res_MS"); //!
  ...
  for (int i = 0; i < nBins; ++ i) {
    Prettify1D(reco_noms_copy[i],kRed,kFullStar,4,kRed,"M [GeV/c^{2}]","1/N dN/dM",0,10,0,0.5);
  ...
  TH1D* hdummycws = new TH1D("hdummycws",";;1/N dN/dM",1,0,10);
